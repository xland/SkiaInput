...
OpenGL2::OpenGL2(WinBase* win) : Context(win), fSurfaceProps(0, kRGB_H_SkPixelGeometry)
{
    backendType = "OpenGL";
    init();
}
...
sk_sp<SkSurface> OpenGL2::getSurface()
{
    if (nullptr == surface) {
...
void OpenGL2::paint(HDC dc) {
    m_grContext->flushAndSubmit(surface.get());
    SwapBuffers(dc);
}
void OpenGL2::textureFromImage(sk_sp<SkImage>& image)
{
